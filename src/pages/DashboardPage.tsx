        ...
        <div className="px-4 py-5 sm:px-6 sm:py-6 max-w-5xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-5">
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-accent text-white rounded-xl text-sm font-medium hover:opacity-90 transition-opacity self-start sm:self-auto shrink-0"
                >
            ...
            <div className="bg-card border border-border rounded-2xl p-4 mb-4">
                ...
                <div className="flex gap-2 flex-wrap">
                    {MOODS.map((mood) => (
                        <button
                            key={mood.value}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-border bg-bg text-xs text-ink2 hover:border-accent hover:bg-accent-pale hover:text-accent transition-all cursor-pointer whitespace-nowrap"
                        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                ...
                <div className="lg:col-span-2 bg-card border border-border rounded-2xl overflow-hidden">
                    <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                        ...
                        <button onClick={() => navigate('/timeline')} className="text-xs text-accent hover:underline">
                            View all →
                        </button>
                    </div>

                    {loading ? (
                        <div className="p-6 text-sm text-muted text-center">Loading entries...</div>
                    ) : recentEntries.length === 0 ? (
                        <div className="p-8 text-center">
                            <div className="text-3xl mb-3">✦</div>
                            <div className="font-lora text-base text-ink mb-1">Your journal awaits</div>
                            <div className="text-xs text-muted mb-4">Write your first entry to get started</div>
                            <button
                                onClick={() => navigate('/write')}
                                className="px-4 py-2 bg-accent text-white rounded-xl text-xs font-medium hover:opacity-90"
                            >
                                Write first entry
                            </button>
                        </div>
                            ...
                            {recentEntries.map((entry) => (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] text-muted">· {entry.wordCount} words</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="flex flex-col gap-4">
                                        ...
                                        <div key={row} className="flex gap-[5px]">
                                            {Array.from({ length: 7 }).map((_, col) => {
                                                ...
                                                return (
                                                    <div
                                                        key={col}
                                                        className={`w-[14px] h-[8px] rounded-[2px] ${hasEntry ? 'bg-accent' : 'bg-surface'}`}
                                                    />
                                                )
                                            })}
                                        </div>
                    ...
                    {/* This Day Last Year */}
                    <div className="flex-1 bg-lav-pale border border-lav/20 rounded-2xl p-4 flex flex-col">
                        <div className="text-[10px] font-semibold text-lav uppercase tracking-wider mb-2.5">
                            This Day Last Year
                        </div>
                        {lastYearEntry ? (
                            <div onClick={() => navigate(`/write/${lastYearEntry.id}`)} className="flex-1 cursor-pointer">
                                <div className="font-lora text-sm font-semibold text-ink mb-1">
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-center py-2">
                                ...
                                <div className="text-xs text-ink2 font-medium mb-1">No entry yet</div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-terra-pale border border-terra/20 rounded-2xl p-5">
                    <div className="text-[10px] font-semibold text-terra uppercase tracking-wider mb-2.5">
                        Today's Prompt
                    </div>
                    <div className="font-lora italic text-sm text-ink leading-relaxed mb-4">
                        "{prompt}"
                    </div>
                    <button onClick={() => navigate('/write')} className="text-xs text-terra font-medium hover:underline">
                        Write about this →
                    </button>
                </div>
                ...
                <div className="bg-card border border-border rounded-2xl p-5 sm:col-span-2 lg:col-span-1">
                    ...
                    <div
                        onClick={() => navigate('/write')}
                        className="w-full min-h-[80px] bg-bg border border-border rounded-xl px-3 py-2.5 text-xs text-muted cursor-pointer hover:border-accent transition-colors"
                    >
                        What's on your mind?
                    </div>
                    <button
                        onClick={() => navigate('/write')}
                        className="w-full mt-2.5 py-2.5 bg-ink text-bg rounded-xl text-xs font-medium hover:opacity-85 transition-opacity"
                    >
                        Open Editor →
                    </button>
                </div>
